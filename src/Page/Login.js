...
const Login = () => {
	...
	const handleGooglesigin = () => {
		signInWithGoogle().then(result => {
			...
			console.log(error)
			setLoading(false)
			toast.error('please cheak your error')
		})

	}
	const handleResat = () => {
		resetPassword(userEmail).then(res => {
			res.console.log(res)
		})
	...
	return (
		<div className=''>
			<div className="w-full max-w-md p-8 space-y-3 rounded-xl dark:bg-gray-900 bg-black mx-auto">
				...
				<form onSubmit={handleSubmit } novalidate="" action="" className="space-y-6 ng-untouched ng-pristine ng-valid">
					...
					<button className="block w-full p-3 text-center rounded-sm dark:text-gray-900 bg-orange-500 text-white">Sign in</button>
				</form>
				<div className="flex items-center pt-4 space-x-1">
					<div className="flex-1 h-px sm:w-16 dark:bg-gray-700"></div>
					<p className="px-3 text-sm dark:text-gray-400 text-white">Login with social accounts</p>
					<div className="flex-1 h-px sm:w-16 dark:bg-gray-700"></div>
				</div>
				<div className="flex justify-center space-x-4 text-white">
					...
					<button onClick={handleGooglesigin} aria-label="Log in with Twitter" className="p-3 rounded-sm">
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className="w-5 h-5 fill-current">
				...
				<p className="text-xs text-center sm:px-6 dark:text-gray-400 text-white">Don't have an account?
					<Link to='/signup'>
